...
"""
Tests for ArFrame._repr_html_()  –  Issue #221

Add these to tests/test_frame.py (they reuse the existing conftest fixtures).
"""
...
def test_repr_html_contains_column_names(sample_csv):
    ...
    for col in frame.columns:       # name, age, email, active
        assert col in out
...
def test_repr_html_truncation_notice_present(large_csv):
    # large_csv has 1000 rows
    frame = ar.read_csv(large_csv)
    out = frame._repr_html_()
    assert "Showing 10 of 1000 rows" in out


def test_repr_html_no_truncation_notice_for_small_frame(sample_csv):
    # sample_csv has 3 rows — well under the 10-row cap
    frame = ar.read_csv(sample_csv)
